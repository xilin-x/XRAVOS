...
class HyperParameters():
    ...
    def parse(self, unknown_arg_ok=False):
        ...
        parser.add_argument('--size_window', help='Neighborhood sampling area size', type=int, default=7)
        ...
        parser.add_argument('--xray_root', help='MOSXAV data root', default='data/MOSXAV')

        parser.add_argument(
            '--stage',
            help='Training stage (0-static images, 1-DAVIS+YouTubeVOS (300K), 2-DAVIS+YouTubeVOS (150K)), 3-MOSXAV (150K)',
            type=int,
            default=0
        )
        ...
        parser.add_argument(
            '-b', '--batch_size', help='Default is dependent on the training stage, see below', default=None, type=int
        )
        parser.add_argument(
            '-i', '--iterations', help='Default is dependent on the training stage, see below', default=None, type=int
        )
        parser.add_argument(
            '--steps', help='Default is dependent on the training stage, see below', nargs="*", default=None, type=int
        )
        parser.add_argument(
            '--start_warm', help='Default is dependent on the training stage, see below', default=20000, type=int
        )
        parser.add_argument('--end_warm', help='Default is dependent on the training stage, see below', default=70000, type=int)
        ...
        if self.args['stage'] == 0:
            ...
            self.args['start_warm'] = none_or_default(self.args['start_warm'], 20000)
            self.args['end_warm'] = none_or_default(self.args['end_warm'], 70000)
            self.args['single_object'] = True
        elif self.args['stage'] == 1:
            # 300K main training for after Static image pretraining
            self.args['lr'] = none_or_default(self.args['lr'], 1e-5)
